...
function inventoryTable(items) {
  ...
  return `<p class="table-count">${items.length} registros · ${items.reduce((s,i)=>s+i.quantity,0)} quantidades cadastradas</p><div class="table-scroll"><table><caption class="sr-only">Inventário do enxoval</caption><thead><tr><th>Item</th><th>Fase / tamanho</th><th>Quantidade</th><th>Detalhes</th><th>Situação</th><th>Ações</th></tr></thead><tbody>${items.map(i=>`<tr><td><strong>${esc(i.original||i.description)}</strong><small>${esc(i.category)} → ${esc(i.type)} → ${esc(i.description)}</small><small>${esc(i.id)}${data.benchmarks.some(b=>matches(i,b))?'':' · Sem meta'}</small></td><td>${esc(i.phase)}<small>${esc(i.size)}</small></td><td><strong>${i.quantity}</strong> ${esc(i.unit)}</td><td><details><summary>${esc([i.brand,i.color].filter(Boolean).join(' · ')||'Ver atributos')}</summary><p>Presente de: ${esc(i.gift||'—')}<br>Fecho/tecido: ${esc(i.fabric||'—')}<br>Observações: ${esc(i.notes||'—')}</p></details></td><td>${esc(i.status)}</td><td><div class="row-actions"><button data-edit="${esc(i.id)}" aria-label="Editar ${esc(i.original||i.description)}">Editar</button><button class="danger" data-delete="${esc(i.id)}" aria-label="Excluir ${esc(i.original||i.description)}">Excluir</button></div></td></tr>`).join('')}</tbody></table></div>`;
}
