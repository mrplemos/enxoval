...
const seed={schemaVersion:1,items:read('inventory'),benchmarks:read('benchmarks')};
const copy=()=>structuredClone(seed);
test('current workbook: 121 records, 419 quantities, 46 independent targets',()=>{
  assert.doesNotThrow(()=>validateData(seed));
  assert.equal(seed.items.length,121);assert.equal(seed.items.reduce((n,i)=>n+i.quantity,0),419);assert.equal(seed.benchmarks.length,46);
  assert.equal(seed.items.at(-1).id,'ITM-0121');assert.equal(seed.items.at(-2).id,'ITM-0120');
});
...
test('phase, category and owned status prevent unrelated items counting',()=>{
  ...
  for(const patch of [{phase:'3–6 meses'},{category:'Outros acessórios'},{status:'Planejado'},{status:'Doado'},{status:'Consumido'}])assert.equal(calculate([{...a,...patch}],[b])[0].have,0);
});
...
test('backup round-trip preserves every attribute and benchmark independently',()=>{
  ...
  assert.deepEqual(parseBackup(JSON.stringify({schemaVersion:1,items:[],benchmarks:[]})),{schemaVersion:1,items:[],benchmarks:[]});
});
test('invalid and future backups reject before mutation',()=>{
  assert.throws(()=>parseBackup('{broken'));
  for(const mutate of [d=>d.schemaVersion=2,d=>d.items.push(d.items[0]),d=>d.items[0].quantity=-1,d=>d.items[0].quantity=1.5,d=>d.items[0].quantity='2',d=>delete d.items[0].brand,d=>d.items[0].status='Unknown',d=>d.benchmarks[0].rule='Anything',d=>d.benchmarks[0].target=null,d=>d.items[0].category='']){const d=copy();mutate(d);assert.throws(()=>validateData(d));}
});
